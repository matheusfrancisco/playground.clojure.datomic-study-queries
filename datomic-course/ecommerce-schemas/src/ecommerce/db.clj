...
;(defn dissoc-nested-db-id [entidade]
;  (walk/prewalk dissoc-db-id entidade))


;;versão simples que não faz recursivamente
;(defn  datomic-para-entidade [entidades]
;  (map dissoc-nested-db-id entidades))
...
;(s/defn um-produto :- model/Produto
;  [db, produto-id :- java.util.UUID]
;  (datomic-para-entidade (d/pull db '[* {:produto/categoria [*]}] [:produto/id produto-id])))


;o maybe permite nil
;nil permite nullpointerexception
;nullpointerexception permite exceptions
;usamos maybe SOMENTE em retorno de função
;E somente quando fizer muito sentido
;isto é ... maybe não é usado em mapas. em mapas uasmos as chaves opcionais.
...
(defn criar-dados-de-examplos [conn]
  ...
  (pprint @(adiciona-produtos! conn [computador, celular, celular-barato, xadrez, jogo] "200.216.222.125"))
...
(s/defn um-produto-com-estoque :- (s/maybe model/Produto)
  [db, produto-id :- java.util.UUID]
